...
const CACHE_VERSION = 'skymonitor-v1.1.9';
...
self.addEventListener('fetch', (e) => {
    ...
    // /radar is a client-side route in the single-page app.
    //
    // Fetch "/" instead of "/index.html". Cloudflare Pages redirects
    // "/index.html" to "/", and returning that redirected response directly
    // from a navigation FetchEvent causes:
    ...
    if (
        ...
        const appShell = new URL('/', self.location.origin);

        e.respondWith(
            fetch(new Request(appShell.href, {
                cache: 'no-store',
                redirect: 'follow'
            }))
                ...
                .then((response) =>
                    response || new Response(
                        'SkyMonitor is temporarily unavailable.',
                        {
                            status: 503,
    ...
    //    Opaque cross-origin responses are not cached.
        ...
        e.respondWith(
            caches.match(e.request).then((cached) => {
                ...
                return fetch(e.request)
                    .then((res) => {
                        // Never cache opaque responses. Browsers pad opaque
                        // cache entries to several MB.
    ...
    // 4. Main HTML and all same-origin assets — network-first.
    ...
    // 5. External API calls — network-only.
    //
    // Weather API URLs contain changing query parameters, so caching them
    // creates unique entries that are never reused. The app's IndexedDB
    // offline logic handles unavailable network requests.
    return;
});
        ...
        (async () => {
            ...
            for (const client of clientList) {
                if (
                    ...
                    const navPromise = ('navigate' in client)
                        ? client.navigate(url)
